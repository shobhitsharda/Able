...
public class CheckMod_SubmitPost {
	...
	boolean submitPost(Connection cn, double postId, String title, String content, String date, String email, String tag)
	{
		try
		{
			String sql = "INSERT INTO DB2ADMIN.POSTDB VALUES(?,?,?,?,?)";
			...
			for (String str : words) 
			{
				...
				String sql2 = "SELECT MAX(TAGID) FROM DB2ADMIN.TAGDB";
				...
				String sql3 = "INSERT INTO DB2ADMIN.POSTTAGDB VALUES(?,?)";
					ps = cn.prepareStatement(sql3);
					ps.setDouble(1, postId);
					ps.setInt(2, mod_tagid);
					ps.executeUpdate();
					
				String sql4 = "INSERT INTO DB2ADMIN.TAGDB VALUES(?,?)";
	...
	double getPostId(Connection cn)
	{
		...
		try
		{
			String sql = "SELECT MAX(POSTID) FROM DB2ADMIN.POSTDB";
			PreparedStatement ps = cn.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
